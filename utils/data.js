...
function getSpots(location, conditions, reports, cityId) {
  ...
  return spots.filter(item => (item.cityId || 'yantai') === selectedCityId).map(item => {
    ...
    return Object.assign({}, item, {
      ...
      conditionsReady: Boolean(localConditions && localConditions.dataReady),
      conditionsBlocked: Boolean(localConditions && localConditions.blocked),
      conditionsReasons: localConditions && localConditions.reasons || [],
      modelNotice: localConditions && localConditions.modelNotice || '',
      tideSource: localConditions && localConditions.tideSource || '数据源待确认',
      tideStation: localConditions && localConditions.tideStation || '',
      tideConfidence: localConditions && localConditions.tideConfidence || '待更新',
      stationDistanceKm: localConditions && localConditions.stationDistanceKm !== undefined ? localConditions.stationDistanceKm : null,
